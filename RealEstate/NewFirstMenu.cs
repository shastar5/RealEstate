...
{
    public partial class NewFirstMenu : MetroFramework.Forms.MetroForm
    {
        public int type;
        public int state;
        ...
        private Boolean checkException()  //검색 에러 핸들
        {
            ...
            {
                MetroMessageBox.Show(Owner,"매매금액을 정해주세요", "콤보박스 미선택", MessageBoxButtons.OK, MessageBoxIcon.Asterisk);
                return false;
            }
            else if (findvalues.sellPriceSize == -1 && !TB_SellPrice.Text.Equals(""))
            {
                MetroMessageBox.Show(Owner, "매매금액 범위를 정해주세요", "콤보박스 미선택", MessageBoxButtons.OK, MessageBoxIcon.Asterisk);
                return false;
            }
            if (findvalues.takeOverPriceSize != -1 && TB_TakeOverPrice.Text.Equals(""))
            {
                MetroMessageBox.Show(Owner, "인수금액을 정해주세요", "인수금액 미입력", MessageBoxButtons.OK, MessageBoxIcon.Asterisk);
                return false;
            }
            else if (findvalues.takeOverPriceSize == -1 && !TB_TakeOverPrice.Text.Equals(""))
            {
                MetroMessageBox.Show(Owner, "인수금액 범위를 정해주세요", "콤보박스 미선택", MessageBoxButtons.OK, MessageBoxIcon.Asterisk);
                return false;
            }
            if (findvalues.IncomeSize != -1 && TB_Income.Text.Equals(""))
            {
                MetroMessageBox.Show(Owner, "월 수입을 정해주세요", "월 수입 미입력", MessageBoxButtons.OK, MessageBoxIcon.Asterisk);
                return false;
            }
            else if (findvalues.IncomeSize == -1 && !TB_Income.Text.Equals(""))
            {
                MetroMessageBox.Show(Owner, "월 수입 범위를 정해주세요", "콤보박스 미선택", MessageBoxButtons.OK, MessageBoxIcon.Asterisk);
                return false;
            }

            if (findvalues.yearPercentSize != -1 && TB_YearPercent.Text.Equals(""))
            {
                MetroMessageBox.Show(Owner, "연 수익율을 정해주세요", "연 수익률 미입력", MessageBoxButtons.OK, MessageBoxIcon.Asterisk);
                return false;
            }
            else if (findvalues.yearPercentSize == -1 && !TB_YearPercent.Text.Equals(""))
            {
                MetroMessageBox.Show(Owner, "연 수익율 범위를 정해주세요", "콤보박스 미선택", MessageBoxButtons.OK, MessageBoxIcon.Asterisk);
                return false;
            }
            if (findvalues.roadwidthSize != -1 && TB_RoadWidth.Text.Equals(""))
            {
                MetroMessageBox.Show(Owner, "도로너비를 정해주세요", "도로너비 미 입력", MessageBoxButtons.OK, MessageBoxIcon.Asterisk);
                return false;
            }
            else if (findvalues.roadwidthSize == -1 && !TB_RoadWidth.Text.Equals(""))
            {
                MetroMessageBox.Show(Owner, "도로너비 범위를 정해주세요", "콤보박스 미선택", MessageBoxButtons.OK, MessageBoxIcon.Asterisk);
                return false;
            }
        ...
        private void typeOnlyNum(object sender, System.Windows.Forms.KeyPressEventArgs e)
        {
        ...
        private void btn_find_Click(object sender, EventArgs e)
        {
            ...
            {
                MetroMessageBox.Show(Owner, "찾을 건물 종류를 선택해주세요","건물 종류 미 선택", MessageBoxButtons.OK, MessageBoxIcon.Asterisk);
            }
            else if (isOpen == 1)
            {
                MetroMessageBox.Show(Owner, "검색 창이 이미 열려 있습니다.","창 중복 방지", MessageBoxButtons.OK, MessageBoxIcon.Asterisk);
            }
            else if (checkException())
            {
                if (ErrorStr2Num == 0)
                {
                    ...
                    MetroMessageBox.Show(Owner, "숫자 입력란에 숫자만 넣어주세요. 다시 확인해주세요.", "숫자 외 입력", MessageBoxButtons.OK, MessageBoxIcon.Asterisk);
        ...
        private void FirstMenu__Load(object sender, EventArgs e)
        {
